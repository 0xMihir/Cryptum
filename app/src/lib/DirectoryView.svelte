...
<div class="d-flex flex-row flex-wrap">
    {#each directory.childrenArray().filter(child => child instanceof Directory) as child (child.name)}
        <FileIcon icon="folder-blue.svg" name={child.name} selected={child.name == selectedFileName} on:click={() => fileClicked(child)} on:contextmenu={(e) => handleRightClick(e, child, false)}/>
    {/each}
    {#each directory.childrenArray().filter(child => child instanceof File) as child (child.name)}
        <FileIcon icon="text-x-generic.svg" name={child.name} selected={child.name == selectedFileName} on:click={() => fileClicked(child)} on:contextmenu={(e) => handleRightClick(e, child, true)}/>
    {/each}
</div>